package mx.edu.utng.orderss.sqlite;
...
import mx.edu.utng.orderss.model.Customer;
import mx.edu.utng.orderss.model.DeptEmp;
import mx.edu.utng.orderss.model.MethodPayment;
import mx.edu.utng.orderss.model.OrderDetail;
import mx.edu.utng.orderss.model.OrderHeader;
import mx.edu.utng.orderss.model.Product;
...
public final class DBOperations {
    ...
    //Operations of  DeptEmp
    public Cursor getDeptEmps(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.DEPT_EMP);
        return  database.rawQuery(sql, null);
    }

    public Cursor getDeptEmpById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.DEPT_EMP,
    ...
    public String insertDeptEmp(DeptEmp deptEmp){
        SQLiteDatabase database = db.getWritableDatabase();
        String idDeptEmp = OrderContract.DeptEmps.generateIdDeptEmp();
        ContentValues values = new ContentValues();
    ...
    public boolean updateDeptEmp(DeptEmp deptEmp){
        SQLiteDatabase database = db.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(OrderContract.DeptEmps.FROMDATE, deptEmp.getFromDate());
        values.put(OrderContract.DeptEmps.TODATE, deptEmp.getToDate());
        ...
        String[] whereArgs = {deptEmp.getIdDep()};
        int result = database.update(OrderDB.Tables.DEPT_EMP,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteDeptEmp(String idDepEmp) {
        SQLiteDatabase database = db.getWritableDatabase();
